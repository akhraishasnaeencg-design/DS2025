...
$\text{Exam Score} = \beta_0 + \beta_1(\text{Math Score}) + \beta_2(\text{Reading Score}) + \beta_3(\text{Writing Score}) + \beta_4(\text{Study Hours}) + \beta_5(\text{Attendance}) + \beta_6(\text{Test Prep}) + \varepsilon$
...
- Les valeurs prédites par le modèle sont très proches des valeurs réelles
- La ligne de régression suit la tendance : y = 0.95x + 3.2
- Coefficient de corrélation : r = 0.92 (très forte corrélation)
- Les points sont bien répartis autour de la ligne de régression, indiquant un bon ajustement
...
$\log\left(\frac{P(\text{Pass})}{1-P(\text{Pass})}\right) = \beta_0 + \beta_1(\text{Math Score}) + \beta_2(\text{Reading Score}) + \beta_3(\text{Study Hours}) + \beta_4(\text{Attendance}) + \beta_5(\text{Test Prep})$
...
- **AUC (Area Under Curve) = 0.972** - Performance excellente
- Le modèle distingue très bien entre les classes Pass et Fail
- Taux de vrais positifs élevé avec un faible taux de faux positifs
- Seuil optimal identifié à 0.52 pour maximiser le F1-Score
...
où :
- Y = Exam Score (variable dépendante)
- X<sub>i</sub> = variables explicatives
- β<sub>i</sub> = coefficients de régression
- ε = terme d'erreur
...
où :
- P(Y=1) = probabilité de réussite (Pass)
